...
class Item_material extends MY_Controller
{
	public function __construct()
	{
		...
		$this->load->model("staff/material_model");
	...
	public function index()
	{
		...
		$data = $datapage + $conf;
		$this->template->load('staff/layout/template', 'staff/material_view',$data);
	}
	...
	public function ajaxPaginationData()
	{
		...
		$this->load->view('staff/ajax_page/ajax_material', $data, false);     
	...
	public function newmaterial()
	{
		...
		$this->template->load('staff/layout/template', 'staff/create_material', $data);
	}
		...
		if(empty($this->input->post('material_variancheck'))) {
			...
			if ($validation_nonvar->run()){
				...
				{   
					...
					redirect('staff/item_material/newmaterial');
				} else {
			...
			public function editmaterial($material_code = null)
			{
				...
				$this->template->load('staff/layout/template', 'staff/edit_material', $data);
			...
			public function delmaterial($material_code = null)
			{
				...
				if ($this->material_model->deletematerial($material_code)) 
				{
					...
					redirect(site_url('staff/item_material'));
				} else {
