...
class Product_card extends MY_Controller
{
	public function __construct()
	{
		...
		$this->load->model("staff/product_model");
	...
	public function index()
	{
		...
		$data                     = $datapage + $conf;
		$this->template->load('staff/layout/template', 'staff/product_view',$data);
	}
	...
	public function ajaxPaginationData()
	{
		...
		$this->load->view('staff/ajax_page/ajax_product', $data, false);     
	...
	public function showbom()
	{
		...
		$this->load->view('staff/modal_view/modal_view_bom', $data);
	}
	...
	public function newproduct()
	{
		...
		$data                              = $datavar+$conf;
		$this->template->load('staff/layout/template', 'staff/create_product', $data);
	}
	...
	public function newbom()
	{
		...
		$this->template->load('staff/layout/template', 'staff/create_bom', $data);
	}
	...
	public function showdetailBOM($product_sku = null)
	{
		...
		$this->template->load('staff/layout/template', 'staff/detail_BOM', $data);
	...
	public function editproductcard($product_code = null)
	{
		...
		$this->template->load('staff/layout/template', 'staff/edit_product', $data);
	...
	public function delproduct($product_code = null)
	{
		...
		if ($this->product_model->deleteproduct($product_code)) 
		{
			...
			redirect(site_url('staff/product_card'));
		} else {
	...
	public function addbillofmaterial()
	{
		...
		if ($validation->run()) {
			...
			redirect('staff/product_card/newbom'); 
		}
	...
	public function addproductcard()
	{
		...
		if(empty($this->input->post('variancheck'))) 
		{
			...
			if ($validation_nonvar->run())
			{
				...
				{   
					$this->session->set_flashdata('msg', show_err_msg('Gagal disimpan.'));
					redirect('staff/product_card/newproduct');
				} else {
					$this->session->set_flashdata('msg', show_succ_msg('Berhasil disimpan.'));
					redirect('staff/product_card/newproduct'); }
				} else {
