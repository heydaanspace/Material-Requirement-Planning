...
class Purchase_order extends MY_Controller
{
	public function __construct()
	{
		...
		$this->load->model("owner/purchase_order_model");
	...
	public function index()
	{
		...
		$data                     = $datapage + $conf;
		$this->template->load('owner/layout/template', 'owner/purchase_order_page',$data);
	}
	...
	public function ajaxPaginationData()
	{
		...
		$this->load->view('owner/ajax_page/ajax_purchase_order_page', $data, false);     
	...
	public function newpo()
	{
		...
		$this->template->load('owner/layout/template', 'owner/create_po', $alldata);
	}
	...
	public function showitemPO()
	{
		...
		$this->load->view('owner/modal_view/modal_view_item_po', $data);
	}
	...
	public function savepo()
	{
		...
		if ($validation->run()) {
			...
			redirect('owner/purchase_order/newpo'); 
		}
	}
	...
	public function showdetailPO($po_id = null)
	{
		...
		$this->template->load('owner/layout/template', 'owner/detail_po_page', $data);
	...
	public function ActdeletePO($po_id = null)
	{
		...
		if ($this->purchase_order_model->deletePO($po_id)) 
		{
			...
			redirect(site_url('owner/purchase_order'));
		} else {
	...
	public function newpo_mrp($mrp_id = null)
	{
		...
		$this->template->load('owner/layout/template', 'owner/create_po_mrp_ref', $data);
	...
	public function newpo_recap_mrp($mo_id = null)
	{
		...
		$conf                           = konfigurasi('Pesanan Pembelian Rujukan');
		$data                           = $value + $conf + $valuearray + $autocode + $datasupplier + $datamaterial;
		$this->template->load('owner/layout/template', 'owner/create_po_recap_mrp_ref', $data);
	...
	public function UpdateStatusReject($detpo_id = null)
	{
		...
		if ($this->purchase_order_model->StatusReject($detpo_id)) 
		{
			...
			$this->session->set_flashdata('msg', show_err_msg('Gagal diperbarui.'));
			redirect(site_url('owners/purchase_order'));
		}
