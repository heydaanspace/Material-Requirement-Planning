...
class Report extends MY_Controller
{
	public function __construct()
	{
		...
		$this->load->model("owner/report_model");
	...
	public function index()
	{
		...
		$this->template->load('owner/layout/template', 'owner/report_page',$data);
	...
	public function stokreport()
	{
		...
		$this->template->load('owner/layout/template', 'owner/stok_report_view',$data);
	...
	public function ajaxInStockReport()
	{
		...
		$this->load->view('owner/ajax_page/ajax_instock_report_page', $data, false);     
	...
	public function ajaxOutStockReport()
	{
		...
		$this->load->view('owner/ajax_page/ajax_outstock_report_page', $data, false);     
	...
	public function ajaxRecapStockReport()
	{
		...
		$this->load->view('owner/ajax_page/ajax_recap_report_page', $data, false);     
	...
	public function printInstock()
	{
		...
		$this->pdfgenerator->load_view('owner/PDF_reporting/instock_report_pdf', $datapage);

		//$this->load->view('owner/pdf_reporting/instock_report_pdf', $datapage);   
	}
	...
	public function printOutstock()
	{
		...
		$this->pdfgenerator->load_view('owner/PDF_reporting/outstock_report_pdf', $datapage);
	...
	public function printRecapstock()
	{
		...
		$this->pdfgenerator->load_view('owner/PDF_reporting/recap_report_pdf', $datapage);
	...
	public function mrpreport()
	{
		...
		$data                     = $apps + $dataMO;
		$this->template->load('owner/layout/template', 'owner/mrp_report_view',$data);
	...
	public function ajaxMrpReport()
	{
		...
		//total rows count
		$totalRec = count($this->report_model->getMrpReport($conditions));
		
		//pagination configuration
		$config['target']      = '#mrpList';
		$config['base_url']    = base_url().'report/ajaxMrpReport';
		$config['total_rows']  = $totalRec;
		$config['per_page']    = $this->perPage;
		$config['link_func']   = 'searchFilter';
		$this->ajax_pagination->initialize($config);
		...
		$this->load->view('owner/ajax_page/ajax_mrp_report_page', $data, false);     
	...
	public function printMRPReport()
	{
		...
		$this->pdfgenerator->load_view('owner/PDF_reporting/mrp_report_pdf', $datapage);
	...
	// 	$conf                      = konfigurasi('Laporan Invoices');
	// 	$datacustomer['customer']  = $this->report_model->getCustomer()->Result();
	// 	$data = $conf + $datacustomer;
	// 	$this->template->load('production/layout/template', 'production/invoices_report_view',$data);

	// }
	// public function ajaxInvoiceProductionReport()
	// {
	// 	$conditions = array();
	...
	// 	//set conditions for search
	// 	$selcustomer = $this->input->post('selcustomer');
	// 	$startDate   = $this->input->post('startDate');
	// 	$endDate = $this->input->post('endDate');
	// 	if(!empty($selcustomer)){
	// 		$conditions['search']['selcustomer'] = $selcustomer;
	// 	}
	// 	if(!empty($startDate)){
	// 		$conditions['search']['startDate']   = $startDate;
	// 	}
	...
	// 	//total rows count
	// 	$totalRec = count($this->report_model->getInvoiceProductionReport($conditions));

	// 	//pagination configuration
	// 	$config['target']      = '#manufacturingList';
	// 	$config['base_url']    = base_url().'report/ajaxInvoiceProductionReport';
	// 	$config['total_rows']  = $totalRec;
	// 	$config['per_page']    = $this->perPage;
	// 	$config['link_func']   = 'searchFilter';
	// 	$this->ajax_pagination->initialize($config);
	...
	// 	$data['manufacturing'] = $this->report_model->getInvoiceProductionReport($conditions);

	// 	$this->load->view('production/ajax_page/ajax_production_invoice_report_page', $data, false);     
