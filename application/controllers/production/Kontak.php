...
class Kontak extends MY_Controller
{
	public function __construct()
	{
		...
		$this->load->model("production/konsumen_model");
		$this->load->model("production/supplier_model");
...
//pagination conf.
	public function index()
	{
		...
		$this->template->load('production/layout/template', 'production/kontak_view', $data);
	...
	public function ajaxPaginationData()
	{
		...
		//load the view
		$this->load->view('production/ajax_page/ajax_konsumen', $custdata, false);
	}
	...
	public function supajaxPaginationData()
	{
		...
		//load the view
		$this->load->view('production/ajax_page/ajax_supplier', $supdata, false);
	}
...
//Customer crud
	public function custfrm()
	{
		...
		$this->template->load('production/layout/template', 'production/create_konsumen', $data);
	}
	public function addkonsumen()
	{
		...
		if ($validation->run()) {
			...
			if ($result > 0) {
				...
				redirect('production/kontak/custfrm');
			} else {
	...
	public function editkonsumen($customer_id = null)
	{
		...
		$data = $value + $conf;
		$this->template->load('production/layout/template', 'production/edit_konsumen', $data);
	}
	...
	public function delkonsumen($customer_id = null)
	{
		...
		if ($this->konsumen_model->delete($customer_id)) {
			...
			redirect(site_url('production/kontak'));
		} else {
...
//supplier Crud
	public function supfrm()
	{
		...
		$this->template->load('production/layout/template', 'production/create_supplier', $data);
	}
	public function addsupplier()
	{
		...
		if ($validation->run()) {
			...
			if ($result > 0) {
				$this->session->set_flashdata('msg', show_succ_msg('Berhasil disimpan.'));
				redirect('production/kontak/supfrm');
			} else {
				$this->session->set_flashdata('msg', show_err_msg('Gagal disimpan.'));
				redirect('production/kontak/supfrm');
			}
	...
	public function editsupplier($supplier_id = null)
	{
		...
		$this->template->load('production/layout/template', 'production/edit_supplier', $data);
	}
