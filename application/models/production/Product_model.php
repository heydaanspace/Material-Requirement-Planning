...
//update function bom begin
public function deleteforupdatebom()
{
    $bom_code  = $this->input->post('ibom_code');
    foreach ($bom_code as $key => $value) 
    {
        $bomcode = $value;
        $this->db->where('bom_code', $bomcode);
        $this->db->delete('bill_of_material');
    }
}
public function saveupdatebillofmaterial()
{
 $product_sku  = $this->input->post('iproductsku');
 $material_sku = $this->input->post('selbom');
 $quantity     = $this->input->post('iqtybom');
 if (!empty($product_sku) && ! empty($material_sku) ) 
 {
    foreach ($material_sku as $key => $value) 
    {
